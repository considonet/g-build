...
const gulpInit = function(config) {
  ...
  // Internal gulp tasks: asset files copy + optimization
  ...
  gulp.task('webp', require("./gulptasks/webp")(config, gulp));
  ...
  gulp.task("watch:dist", done => {
    ...
    if(config.paths.output.css !== false) {
      ...
      gulp.watch([ path.posix.join(config.paths.input.css, '/**/*'), path.posix.join(`!${config.paths.input.css}`, '/**/*.{scss,js,jsx,ts,tsx,html,vue,map}') ], gulp.parallel("assets:dist", "webp"));
    }
    ...
    if(config.paths.output.ejs !== false) {
      gulp.watch(path.posix.join(config.paths.input.ejs, '/**/*.ejs'), gulp.series('ejs'));
    }
  ...
  gulp.task("watch:dev", done => {
    ...
    if(config.paths.output.css !== false) {
      ...
      gulp.watch([ path.posix.join(config.paths.input.css, '/**/*'), path.posix.join(`!${config.paths.input.css}`, '/**/*.{scss,js,jsx,ts,tsx,html,vue,map}') ], gulp.series(gulp.parallel("assets:dev", "webp"), reloadBrowserSync));
    }

    if(config.paths.output.misc !== false) {
      gulp.watch(path.posix.join(config.paths.input.misc, '/**/*'), gulp.series('miscassets', reloadBrowserSync));
    }
  ...
  // External gulp tasks
  gulp.task('build', gulp.series("clean", "assets:dist", gulp.parallel("webp", "miscassets", "styles:dist", "webpack:dist", "ejs")));
  gulp.task("serve", gulp.series("clean", "assets:dev", "webp", "miscassets", "styles:dev", "ejs", "watch:dev", "webpack:watch", "php", "browsersync"));
  gulp.task("watch", gulp.series("clean", "assets:dist", "webp", "miscassets", "styles:dist", "ejs", gulp.parallel("webpack:watchdist", "watch:dist")));
  gulp.task("fullassets", gulp.series("assets:dist", "webp", "miscassets"));
...
const init = function(configFile, noGulp = false) {
  ...
  Object.keys(conf.jsEntries).forEach(k => {
    if(conf.jsEntries[k].indexOf(".js") !== -1) {
      logger.error(`Unsupported config feature (3.x+): Entry point ${k} output path (${conf.jsEntries[k]}) contains file extension.`);
    }
  });
  ...
  // TODO: remove in 4.x
  if(conf.targetBrowsers !== false) {
    logger.warn("Config key targetBrowsers is deprecated and will be removed starting from version 4.x. Please use browserslist key in package.json or .browserslistrc file. For more details see here: https://github.com/browserslist/browserslist#queries")
  }
