...
const gulpInit = function(config) {
  ...
  gulp.task("watch:dev", done => {
    ...
    if(config.paths.output.css !== false) {
      ...
      gulp.watch([ path.posix.join(config.paths.input.css, '/**/*'), path.posix.join(`!${config.paths.input.css}`, '/**/*.{scss,js,jsx,ts,tsx,html,vue,map}') ], gulp.series('assets:dev', reloadBrowserSync));
    }
  ...
  gulp.task("serve", gulp.series("clean", "assets:dev", "miscassets", "styles:dev", "ejs", "watch:dev", "webpack:watch", "php", "browsersync"));
  gulp.task("watch", gulp.series("clean", "assets:dist", "miscassets", "styles:dist", "ejs", gulp.parallel("webpack:watchdist", "watch:dist")));
