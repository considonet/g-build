...
const gulpInit = function(config) {
  ...
  gulp.task("watch:dist", done => {
    gulp.watch([ path.join(config.paths.input.css, '/**/*', path.join(`!${config.paths.input.css}`, '/**/*.{scss,js,jsx,ts,tsx,html,vue,map}')) ], gulp.series('assets', "miscassets")); //TODO: it doesnt watch/react
