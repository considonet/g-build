...
const gulpInit = function(config) {
  ...
  gulp.task("watch:dist", done => {
    gulp.watch([ path.posix.join(config.paths.input.css, '/**/*', path.posix.join(`!${config.paths.input.css}`, '/**/*.{scss,js,jsx,ts,tsx,html,vue,map}')) ], gulp.series('assets', "miscassets")); //TODO: it doesnt watch/react
  ...
  // External gulp tasks
  gulp.task('build', gulp.series("clean", "assets:dist", gulp.parallel("miscassets", 'styles:dist', 'webpack:dist')));
